#!/usr/bin/env python3
"""Create and validate privacy-minimal state for leadership-coach.

Long-term state stores ONLY: schedule, coaching intent (direction / stage /
de-identified scene tag), last diagnosis checkpoint, and irreversible source
fingerprints. Any raw Feishu id / body / token in state is a schema violation.
"""
...
def initial_state(report_time: str, timezone: str) -> dict[str, Any]:
    ...
    return {
        ...
        "scan": {"baseline_days": 30, "last_success_at": None, "fingerprints": []},
    }
...
def validate_state(state: dict[str, Any]) -> None:
    ...
    if not isinstance(directions, list) or not all(isinstance(item, str) and item.strip() for item in directions):
        raise StateError("intent.directions must be an array of non-empty strings")
    stage = intent.get("stage")
    if stage is not None and stage not in STAGES:
        raise StateError(f"intent.stage must be one of {STAGES} or null")
    scene_tag = intent.get("scene_tag")
    if scene_tag is not None and (not isinstance(scene_tag, str) or len(scene_tag) > 40):
        raise StateError("intent.scene_tag must be a short string (<=40 chars) or null")
    if not isinstance(scan, dict) or scan.get("baseline_days") != 30:
        raise StateError("scan.baseline_days must be 30")
...
def command_set_intent(args: argparse.Namespace) -> int:
    state = load_state(args.path)
    directions = list(dict.fromkeys(state["intent"]["directions"] + args.direction))
    state["intent"]["directions"] = directions
    if args.stage is not None:
        if args.stage not in STAGES:
            raise StateError(f"stage must be one of {STAGES}")
        state["intent"]["stage"] = args.stage
    if args.scene_tag is not None:
        state["intent"]["scene_tag"] = args.scene_tag
    if args.confirm:
        if not directions or not state["intent"]["stage"]:
            raise StateError("at least one direction and a stage are required before confirmation")
    ...
    print(
        json.dumps(
            {
                "ok": True,
                "confirmed": state["intent"]["confirmed"],
...
def parser() -> argparse.ArgumentParser:
    ...
    intent = commands.add_parser("set-intent")
    intent.add_argument("--direction", action="append", default=[])
    intent.add_argument("--stage")
    intent.add_argument("--scene-tag")
    intent.add_argument("--confirm", action="store_true")
    intent.set_defaults(func=command_set_intent)
    return root
