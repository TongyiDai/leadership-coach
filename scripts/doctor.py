#!/usr/bin/env python3
"""Read-only readiness check for the leadership-coach skill.

Verifies lark-cli is installed, the identity is a verified user, and reports
which evidence sources are available. Meeting transcripts (minutes / notes) are
the priority source for this skill, so they are treated as core.
"""
...
class Capability:
    def __init__(self, level: str, all_of: tuple[str, ...] = (), any_of: tuple[str, ...] = ()) -> None:
        self.level = level
        self.all_of = all_of
        self.any_of = any_of


# Priority source for leadership-coach = meeting transcripts (minutes / notes).
CAPABILITIES = {
    "meetings": Capability("core", all_of=("vc:meeting.search:read",)),
    "meeting_notes": Capability("core", all_of=("vc:note:read",)),
    "minutes": Capability(
        "core",
    ...
    "messages_search": Capability("recommended", all_of=("search:message", "im:message:readonly")),
    "documents_search": Capability("recommended", all_of=("search:docs:read",)),
...
def evaluate_capabilities(scopes: set[str]) -> dict[str, dict[str, Any]]:
    ...
    for name, requirement in CAPABILITIES.items():
        ...
        result[name] = {"available": not missing, "level": requirement.level, "missing": missing}
    return result
...
def inspect_auth(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # Priority meeting-transcript sources gate the core scan.
    transcript_sources = ("meetings", "meeting_notes", "minutes")
    has_transcript = any(capabilities[name]["available"] for name in transcript_sources)
    other_sources = ("messages_search", "documents_search", "okr", "tasks")
    available_sources = [
        name for name in (*transcript_sources, *other_sources) if capabilities[name]["available"]
    ]
    ...
    return {
        ...
        "has_meeting_transcript_source": has_transcript,
...
def main() -> int:
    ...
    if not live:
        result["ok"] = True
        result["warnings"].append("static check only; pass --live to verify identity and scopes")
        print(json.dumps(result, ensure_ascii=False, indent=2))
        return 0
    ...
    result["probe"] = public_probe(probe)
    result["ok"] = result["ready_for_core_scan"]
    if result["has_meeting_transcript_source"] is False:
        result["warnings"].append(
            "no meeting-transcript source available; this is the priority evidence source and the diagnosis will be weaker without it"
        )
