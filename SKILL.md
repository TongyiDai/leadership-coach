...
description: |
  ...
  可操作改进建议，可从共同会议里找出他人正向可学的行为片段作为可模仿的榜样，并可基于诊断结果做
  情景演练和 GROW 拉底式教练。支持定期运行（默认每周一上午
  10:30 复盘上一周的领导力相关内容）。用户提到领导力陪练、领导力教练、提升领导力、想更有影响力、
  我沟通/协作有什么问题、复盘我在会上的发言、找个领导力榜样向谁学、练一场难搞的对话、每周领导力复盘时使用。
  仅限本人自愿自读：只诊断本人的行为；找榜样时只指出他人正向可学的行为片段，不对他人打分/排名/画像、
...
- **只诊断本人的行为**，不对他人（下属、同事、协作方）做个人画像或评价。
  - **一个受控例外——榜样片段（阶段 3.5）**：为了本人学习，可以指出共同会议里他人**做得好的、值得学的具体行为片段**（可含正向框架归因）。但**只提正向可学点**，绝不给他人打分/排名/贴"领导力强弱"标签、不做负面评价、不推测动机；焦点始终是"我怎么学"，他人只作可借鉴的行为样本。
- **不做绩效、晋升、调薪、离职、纪律判断**，不产出对他人的人格/忠诚度/心理状态标签。
- 不用于员工监控、HR 评估、招聘、教育测评或第三方画像。
- 会议逐字记录里**他人的发言，在诊断本人问题时只用于还原上下文并去标识**；仅在用户明确开启阶段 3.5 榜样片段时，才保留他人的正向行为原句供学习（是否显示真名由用户决定，默认不固化姓名到报告/状态/记忆）。
...
阶段0 身份与边界闸门  →  阶段1 开场多轮对齐(教练契约)  →  阶段2 建立领导力工作地图(先确认)
     →  阶段3 领导力诊断(核心特色)  →  阶段3.5 榜样片段(可选)  →  阶段4 陪练(可选)  →  可选:落盘 + 记忆锚点
```
...
可选：在首要方向的改进动作后，按 `references/reading-list.md` 顺手推荐**一本**对口的经典书（一次诊断最多 2 本，克制使用，不掉书袋）。

## 阶段 3.5 · 榜样片段（可选，向身边的活榜样学）

诊断出用户某方向薄弱后，问一句："要不要我从你们一起开过的会里，找找**别人在这个点上做得好的具体做法**，给你一个可模仿的榜样？"用户要，才进入。完整规则见 `references/role-model.md`。

- **取证**：复用阶段 2 已取的共同会议逐字稿，在用户薄弱的那个方向上，找**他人的正向行为片段**（一个开放式好问题、一次先接情绪再给反馈、一次把分歧转成讨论……）。
- **产出**每条榜样片段固定结构：`可模仿的行为 + 真实场景原句(他人那句话/那个做法) + 为什么有效(挂框架) + 对照你的差距 + 你下次可以怎么试`。
- **署名**：默认可指名（数据本就在用户参与的会里）；但**只呈现正向可学点**，绝不打分/排名/贴"领导力强弱"标签、不负面评价、不推测动机。姓名**不写入报告文件/state/记忆**，只在对话里说。
- **边界自洽**：焦点是"我怎么学"，他人只是可借鉴的行为样本。若用户其实想要的是"评判某人行不行"，明确拒绝并拉回学习目的。

## 阶段 4 · 陪练（可选副线）

征询用户是否要练。场景默认取自阶段 1 的真实场景 + 阶段 3 诊断出的薄弱项。也可**把阶段 3.5 的榜样做法设为演练目标**（"像那位同事那样，先问一个开放问题再给结论"）。两种形态见 `references/sparring.md`：
